...
interface AppAction {
  key: string; label: string; template_name: string; display_name: string; body: string; category: string;
  footer: string; header_type: string; header_content: string;
  button_type: string; buttons: { type: string; text: string; url?: string; phone?: string }[];
...
const APP_ACTIONS: AppAction[] = [
  { key: "", label: "— Select Action —", template_name: "", display_name: "", body: "", category: "UTILITY",
    footer: "", header_type: "NONE", header_content: "", button_type: "NONE", buttons: [], sample_values: {}, tags: "", notes: "" },
  { key: "study_reminder", label: "📚 Study Reminder", template_name: "study_reminder", display_name: "Study Reminder",
    body: "Hi {{1}}, it's time to review *{{2}}*!\n\nYour memory score: {{3}}%\nLast studied: {{4}} ago\n\nA quick 5-minute revision now will boost your retention significantly.",
    category: "UTILITY", footer: "BrainBoost AI • Smart Learning", header_type: "TEXT", header_content: "📚 Study Reminder",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Start Revision", url: `${APP_URL}/app?tab=brain&action=review` }],
    sample_values: { "{{1}}": "Rahul", "{{2}}": "Organic Chemistry", "{{3}}": "45", "{{4}}": "3 days" }, tags: "study,reminder,revision",
    notes: "Triggered when a topic is due for revision based on memory decay algorithm. Variables: name, topic, memory_score, last_studied." },
  { key: "forget_risk", label: "⚠️ Forget Risk Alert", template_name: "forget_risk_alert", display_name: "Forget Risk Alert",
    body: "Hi {{1}}, your memory for *{{2}}* is dropping.\n\nCurrent score: {{3}}%\nPredicted forget date: {{4}}\n\nA quick revision now can prevent memory loss.",
    category: "UTILITY", footer: "BrainBoost AI • Memory Protection", header_type: "TEXT", header_content: "⚠️ Memory Alert",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Revise Now", url: `${APP_URL}/app?tab=brain&action=review` }],
    sample_values: { "{{1}}": "Priya", "{{2}}": "Thermodynamics", "{{3}}": "32", "{{4}}": "Feb 20, 2026" }, tags: "alert,memory,risk",
    notes: "Sent when memory score drops below threshold. High urgency — drives immediate action." },
  { key: "risk_digest", label: "📊 Daily Risk Digest", template_name: "daily_risk_digest", display_name: "Daily Risk Digest",
    body: "Hi {{1}}, here's your daily risk summary.\n\nTopics at risk: {{2}}\nWeakest topic: *{{3}}* ({{4}}%)\nAverage score: {{5}}%\n\nFocus on weak topics today for best results.",
    category: "UTILITY", footer: "BrainBoost AI • Daily Digest", header_type: "TEXT", header_content: "📊 Daily Risk Report",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "View Dashboard", url: `${APP_URL}/app?tab=progress` }],
    sample_values: { "{{1}}": "Amit", "{{2}}": "5", "{{3}}": "Physics Optics", "{{4}}": "28", "{{5}}": "62" }, tags: "digest,risk,daily",
    notes: "Daily automated digest of at-risk topics. Sent once per day, typically morning." },
  { key: "streak_milestone", label: "🔥 Streak Milestone", template_name: "streak_milestone", display_name: "Streak Milestone",
    body: "Congratulations {{1}}!\n\nYou've achieved a *{{2}}-day study streak*!\nTotal sessions: {{3}}\nLeaderboard rank: #{{4}}\n\nKeep up the amazing work!",
    category: "MARKETING", footer: "BrainBoost AI • Achievements", header_type: "TEXT", header_content: "🔥 Streak Achievement!",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "View My Streak", url: `${APP_URL}/app?tab=you&section=streak` }],
    sample_values: { "{{1}}": "Sneha", "{{2}}": "30", "{{3}}": "95", "{{4}}": "12" }, tags: "streak,milestone,achievement",
    notes: "Celebrates streak milestones (7, 14, 30, 60, 100 days). Great for engagement and retention." },
  { key: "streak_break_warning", label: "💔 Streak Break Warning", template_name: "streak_break_warning", display_name: "Streak Break Warning",
    body: "Hi {{1}}, your *{{2}}-day streak* is about to break!\n\nTime remaining: {{3}} hours\n\nA quick 2-minute session will save your streak. Don't lose your progress!",
    category: "UTILITY", footer: "BrainBoost AI • Streak Protection", header_type: "TEXT", header_content: "⏰ Streak Alert",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Save My Streak", url: `${APP_URL}/app?tab=home&action=quick-study` }],
    sample_values: { "{{1}}": "Arjun", "{{2}}": "15", "{{3}}": "4" }, tags: "streak,warning,urgent",
    notes: "Urgent notification when streak is about to expire. Sent when <6 hours remaining." },
  { key: "brain_update_reminder", label: "🧠 Brain Update Nudge", template_name: "brain_update_reminder", display_name: "Brain Update Nudge",
    body: "Hi {{1}}, it's been {{2}} hours since your last brain update.\n\nPending topics: {{3}}\nBrain score: {{4}}%\n\nUpdate now to keep your learning on track.",
    category: "UTILITY", footer: "BrainBoost AI • Brain Health", header_type: "TEXT", header_content: "🧠 Brain Update Needed",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Update Brain", url: `${APP_URL}/app?tab=brain&action=update` }],
    sample_values: { "{{1}}": "Kavya", "{{2}}": "36", "{{3}}": "8", "{{4}}": "71" }, tags: "brain,update,reminder",
    notes: "Sent when user hasn't updated brain in 24+ hours. Encourages regular engagement." },
  { key: "daily_briefing", label: "🌅 Daily Briefing", template_name: "daily_briefing", display_name: "Daily Briefing",
    body: "Good morning {{1}}!\n\nToday's plan: {{2}} topics\nBrain score: {{3}}%\nStreak: {{4}} days\nFocus topic: *{{5}}*\n\nStart your day with maximum impact!",
    category: "UTILITY", footer: "BrainBoost AI • Morning Briefing", header_type: "TEXT", header_content: "🌅 Your Daily Briefing",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Start Studying", url: `${APP_URL}/app?tab=home` }],
    sample_values: { "{{1}}": "Rohan", "{{2}}": "6", "{{3}}": "78", "{{4}}": "12", "{{5}}": "Calculus" }, tags: "briefing,daily,morning",
    notes: "Morning briefing sent daily at user's preferred time. Summarizes today's study plan." },
  { key: "brain_missions", label: "🎯 Brain Missions", template_name: "brain_missions", display_name: "Brain Missions",
    body: "Hi {{1}}, you have a new mission!\n\nMission: *{{2}}*\nType: {{3}}\nReward: {{4}}\nDeadline: {{5}}\n\nAccept the challenge and level up your learning!",
    category: "MARKETING", footer: "BrainBoost AI • Missions", header_type: "TEXT", header_content: "🎯 New Mission Available",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "View Missions", url: `${APP_URL}/app?tab=brain&section=missions` }],
    sample_values: { "{{1}}": "Meera", "{{2}}": "Master Physics Waves", "{{3}}": "Weekly", "{{4}}": "50 XP", "{{5}}": "Sunday" }, tags: "mission,challenge,gamification",
    notes: "Gamification trigger — sent when AI generates new learning missions for the user." },
  { key: "weekly_insights", label: "📈 Weekly Insights", template_name: "weekly_insights_report", display_name: "Weekly Insights Report",
    body: "Hi {{1}}, here's your weekly summary.\n\nTopics studied: {{2}}\nHours invested: {{3}}\nAccuracy: {{4}}%\nRank change: {{5}}\nTop improvement: *{{6}}*",
    category: "UTILITY", footer: "BrainBoost AI • Weekly Report", header_type: "TEXT", header_content: "📈 Weekly Learning Report",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "View Full Report", url: `${APP_URL}/app?tab=progress&section=weekly` }],
    sample_values: { "{{1}}": "Vikram", "{{2}}": "14", "{{3}}": "8.5", "{{4}}": "82", "{{5}}": "+3 positions", "{{6}}": "Biology" }, tags: "weekly,insights,report",
    notes: "Weekly automated report sent every Sunday. Covers performance, rank changes, and recommendations." },
  { key: "exam_countdown", label: "⏰ Exam Countdown", template_name: "exam_countdown", display_name: "Exam Countdown",
    body: "Hi {{1}}, *{{2}}* is approaching!\n\nDays remaining: {{3}}\nReadiness: {{4}}%\nTopics remaining: {{5}}\nDaily target: {{6}} topics\n\nStay focused, you've got this!",
    category: "UTILITY", footer: "BrainBoost AI • Exam Prep", header_type: "TEXT", header_content: "⏰ Exam Countdown",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Study Now", url: `${APP_URL}/app?tab=action&action=exam` }],
    sample_values: { "{{1}}": "Ananya", "{{2}}": "JEE Mains", "{{3}}": "15", "{{4}}": "68", "{{5}}": "12", "{{6}}": "3" }, tags: "exam,countdown,preparation",
    notes: "Triggered when exam date is within 30 days. Frequency increases as exam approaches." },
  { key: "burnout_detection", label: "😮‍💨 Burnout Alert", template_name: "burnout_alert", display_name: "Burnout Alert",
    body: "Hi {{1}}, we noticed you might be fatigued.\n\nFatigue score: {{2}}%\nSession duration: {{3}} mins\n\nWe recommend a break. Your optimal study time is {{4}}. Take care of yourself!",
    category: "UTILITY", footer: "BrainBoost AI • Wellness", header_type: "TEXT", header_content: "🌿 Wellness Check",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "View Wellness", url: `${APP_URL}/app?tab=you&section=wellness` }],
    sample_values: { "{{1}}": "Deepa", "{{2}}": "85", "{{3}}": "120", "{{4}}": "6:00 PM" }, tags: "burnout,wellness,health",
    notes: "AI-triggered when burnout detection system detects high fatigue levels during study sessions." },
  { key: "subscription_expiry", label: "💳 Subscription Expiry", template_name: "subscription_expiry", display_name: "Subscription Expiry",
    body: "Hi {{1}}, your *{{2}}* plan expires in {{3}} days.\n\nExpiry date: {{4}}\nRenewal price: {{5}}\n\nRenew now to continue uninterrupted learning!",
    category: "MARKETING", footer: "BrainBoost AI • Billing", header_type: "TEXT", header_content: "💳 Subscription Reminder",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Renew Now", url: `${APP_URL}/profile?section=subscription` }],
    sample_values: { "{{1}}": "Sanjay", "{{2}}": "Pro", "{{3}}": "5", "{{4}}": "Feb 22, 2026", "{{5}}": "₹299/mo" }, tags: "subscription,billing,renewal",
    notes: "Sent 7, 3, and 1 day(s) before subscription expiry. Critical for retention." },
  { key: "new_user_welcome", label: "👋 Welcome Message", template_name: "new_user_welcome", display_name: "Welcome Message",
    body: "Welcome to BrainBoost, {{1}}!\n\nWe're excited to have you. Start your learning journey with *{{2}}*.\n\nJoin {{3}} other learners in our community.\n\nLet's make learning smarter together!",
    category: "MARKETING", footer: "BrainBoost AI • Welcome", header_type: "TEXT", header_content: "👋 Welcome to BrainBoost!",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Get Started", url: `${APP_URL}/onboarding` }],
    sample_values: { "{{1}}": "Student", "{{2}}": "Your First Topic", "{{3}}": "10,000" }, tags: "welcome,onboarding,new_user",
    notes: "Sent immediately after user signs up. First touchpoint — sets the tone for the entire journey." },
  { key: "inactivity_nudge", label: "💤 Inactivity Nudge", template_name: "inactivity_nudge", display_name: "Inactivity Nudge",
    body: "Hi {{1}}, we miss you!\n\nInactive for: {{2}} days\nMemory drop: {{3}}%\nDecaying topics: {{4}}\n\nYour {{5}} friends are still learning. Come back and catch up!",
    category: "MARKETING", footer: "BrainBoost AI • We Miss You", header_type: "TEXT", header_content: "💤 We Miss You!",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Come Back", url: `${APP_URL}/app?tab=home&action=quick-study` }],
    sample_values: { "{{1}}": "Ravi", "{{2}}": "5", "{{3}}": "18", "{{4}}": "7", "{{5}}": "12" }, tags: "inactivity,reengagement,nudge",
    notes: "Triggered after 3+ days of inactivity. Uses social proof (friends active) to motivate return." },
  { key: "leaderboard_rank_up", label: "🏅 Rank Up", template_name: "leaderboard_rank_up", display_name: "Leaderboard Rank Up",
    body: "Congratulations {{1}}!\n\nYou climbed from #{{2}} to #{{3}}!\nPosition jump: {{4}}\nYou're in the top {{5}}%!\n\nKeep rising on the leaderboard!",
    category: "MARKETING", footer: "BrainBoost AI • Leaderboard", header_type: "TEXT", header_content: "🏅 Rank Up!",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "View Leaderboard", url: `${APP_URL}/app?tab=you&section=leaderboard` }],
    sample_values: { "{{1}}": "Nikhil", "{{2}}": "25", "{{3}}": "18", "{{4}}": "7", "{{5}}": "10" }, tags: "leaderboard,rank,achievement",
    notes: "Sent when user's leaderboard rank improves by 3+ positions. Encourages competitive engagement." },
  { key: "promo_seasonal", label: "🎉 Seasonal Promo", template_name: "promo_seasonal", display_name: "Seasonal Promo",
    body: "Hi {{1}}, we have a special offer!\n\n*{{2}}*\nDiscount: {{3}}% off\nValid until: {{4}}\nCode: *{{5}}*\n\nDon't miss this limited-time offer!",
    category: "MARKETING", footer: "BrainBoost AI • Offers", header_type: "TEXT", header_content: "🎉 Special Offer!",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Claim Offer", url: `${APP_URL}/profile?section=subscription&promo={{5}}` }],
    sample_values: { "{{1}}": "Pooja", "{{2}}": "Summer Sale", "{{3}}": "40", "{{4}}": "Mar 15, 2026", "{{5}}": "SUMMER40" }, tags: "promo,seasonal,discount",
    notes: "Seasonal promotional template. Update offer details, dates, and promo code before each campaign." },
  { key: "promo_upgrade", label: "⬆️ Upgrade Promo", template_name: "promo_upgrade", display_name: "Upgrade Promo",
    body: "Hi {{1}}, level up your learning!\n\nCurrent plan: {{2}}\nUpgrade to: *{{3}}*\nPrice: {{4}}/month\nSave: {{5}}%\n\nUnlock premium features today!",
    category: "MARKETING", footer: "BrainBoost AI • Upgrade", header_type: "TEXT", header_content: "⬆️ Upgrade Your Plan",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Upgrade Now", url: `${APP_URL}/profile?section=subscription&upgrade=true` }],
    sample_values: { "{{1}}": "Arun", "{{2}}": "Free", "{{3}}": "Pro", "{{4}}": "₹299", "{{5}}": "25" }, tags: "promo,upgrade,plan",
    notes: "Targets free-tier users with upgrade offers. Best sent after user hits a feature gate." },
  { key: "promo_referral", label: "🤝 Referral Promo", template_name: "promo_referral", display_name: "Referral Promo",
    body: "Hi {{1}}, share the love of learning!\n\nEarn *{{2}}* per referral\nYour code: *{{3}}*\nFriends joined: {{4}}\n\nInvite more friends and earn rewards!",
    category: "MARKETING", footer: "BrainBoost AI • Referrals", header_type: "TEXT", header_content: "🤝 Refer & Earn",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Invite Friends", url: `${APP_URL}/profile?section=referral` }],
    sample_values: { "{{1}}": "Divya", "{{2}}": "₹100", "{{3}}": "DIVYA2026", "{{4}}": "3" }, tags: "referral,invite,rewards",
    notes: "Referral program template. Sent to active users with 7+ day streaks to maximize viral growth." },
  { key: "promo_reengagement", label: "🔄 Re-engagement", template_name: "promo_reengagement", display_name: "Re-engagement Offer",
    body: "Hi {{1}}, we'd love to have you back!\n\nInactive: {{2}} days\nMemory drop: {{3}}%\nSpecial offer: *{{4}}*\nCode: *{{5}}*\n\nYour {{6}} friends are still learning!",
    category: "MARKETING", footer: "BrainBoost AI • Come Back", header_type: "TEXT", header_content: "🔄 Welcome Back Offer",
    button_type: "CALL_TO_ACTION", buttons: [{ type: "URL", text: "Come Back & Save", url: `${APP_URL}/app?tab=home&promo={{5}}` }],
    sample_values: { "{{1}}": "Kiran", "{{2}}": "10", "{{3}}": "25", "{{4}}": "50% Off Pro", "{{5}}": "COMEBACK50", "{{6}}": "8" }, tags: "reengagement,comeback,offer",
    notes: "Win-back template for churned users (10+ days inactive). Combines discount with social proof." },
];
              ...
              <div className="space-y-3">
                <div className="bg-card border border-border rounded-2xl p-5 space-y-3">
                  ...
                  <div>
                      ...
                      onChange={e => {
                        ...
                        if (action && action.key) {
                          setForm({
                            ...
                            language: "en",
                            header_type: action.header_type,
                            header_content: action.header_content,
                            footer_text: action.footer,
                            button_type: action.button_type,
                            buttons: action.buttons,
                            sample_values: action.sample_values,
                            tags: action.tags,
                            notes: action.notes,
                          });
                        }
                    ...
                    <p className="text-[9px] text-muted-foreground mt-0.5">Select an action to auto-fill all fields — ready to submit to META</p>
                  </div>
