...
export default function STQEngineAdmin() {
  ...
  const openDropdown = () => {
    if (examBtnRef.current) {
      const rect = examBtnRef.current.getBoundingClientRect();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Exam Dropdown Portal - fixed position */}
      <AnimatePresence>
        {showExamDropdown && (
          <>
            <div className="fixed inset-0 z-[999]" onClick={() => setShowExamDropdown(false)} />
            <motion.div
              initial={{ opacity: 0, y: -5, scale: 0.97 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: -5, scale: 0.97 }}
              transition={{ duration: 0.15 }}
              className="fixed z-[1000] w-56 rounded-xl border border-border bg-popover shadow-2xl"
              style={{ top: dropdownPos.top, right: dropdownPos.right }}
            >
              <div className="max-h-72 overflow-y-auto overscroll-contain py-1">
                {["Government", "Entrance", "Global"].map(cat => (
                  <div key={cat}>
                    <p className="px-3 pt-3 pb-1.5 text-[9px] font-bold uppercase tracking-widest text-muted-foreground/60 sticky top-0 bg-popover">{cat}</p>
                    {STQ_EXAM_TYPES.filter(e => e.category === cat).map(e => (
                      ...
                      >
                        {examType === e.id ? <Check className="w-3.5 h-3.5 shrink-0" /> : <span className="w-3.5 shrink-0" />}
