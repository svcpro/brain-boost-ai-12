...
const ACRY_API_ROUTES = [
  ...
  // ── Action Tab API: Session Blueprint (Step 1b — Preview before starting) ──
  { group: "Action Tab API", path: "action-tab-api/session-blueprint", method: "POST", desc: "📋 SESSION BLUEPRINT: Lightweight preview of what the AI session will look like BEFORE starting. Shows AI-selected topic, session phases, expected outcomes (stability gain, rank impact), duration, difficulty, and CTA. Does NOT create a session — call start-focus-session after user confirms. Supports all 6 modes: focus, revision, mock, emergency, current-affairs, intel-practice.", auth: true,
    request: { action: "session-blueprint", mode: "focus", topic_id: "uuid-or-omit-for-auto-pick" },
    response: { blueprint: { mode: "focus", mode_label: "Focus Study Mode", description: "4-phase deep work blueprint" }, ai_selected_topic: { id: "uuid", name: "Third Law (Action and Reaction)", subject: "Newton's Laws of Motion", memory_strength: 0, revision_count: 2, last_revision_date: "ISO_DATE", predicted_drop_date: "ISO_DATE" }, session_config: { duration_minutes: 30, difficulty: "easy", difficulty_label: "Easy", total_phases: 4, total_questions: 10, scoring: { correct: 4, incorrect: -1, unanswered: 0 } }, expected_outcomes: { stability_gain: "+8-15%", stability_gain_min: 8, stability_gain_max: 15, rank_impact: "+150-400 ranks", rank_impact_min: 150, rank_impact_max: 400, current_stability: 0, projected_stability: 15 }, session_phases: [{ type: "recall", title: "Active Recall", description: "Retrieve key concepts from memory without aids", duration: 8, icon: "brain" }, { type: "reinforcement", title: "Concept Reinforcement", description: "Strengthen weak neural pathways with targeted content", duration: 9, icon: "refresh-cw" }, { type: "mcq", title: "Adaptive Assessment", description: "AI-calibrated questions matching your current level", duration: 9, icon: "target" }, { type: "review", title: "Consolidation Review", description: "Lock in gains with spaced review of key takeaways", duration: 4, icon: "book-open" }], exam_context: { exam_type: "NEET", days_to_exam: 45, urgency: "moderate" }, cta: { label: "Enter Focus Mode", next_action: "start-focus-session", payload: { mode: "focus", topic_id: "uuid" } }, meta: { generated_at: "ISO_DATE" } }
  },

  // ── Action Tab API: Start Focus Session (Step 2 — Quiz Bootstrap) ──
  { group: "Action Tab API", path: "action-tab-api/start-focus-session", method: "POST", desc: "🎯 STEP 2: Start a study session for ANY mode (focus/revision/mock/emergency/current-affairs/intel-practice). Creates session, auto-picks weakest topic if no topic_id, fetches AI-generated MCQs, returns session_config with scoring rules & UI features. Single call bootstraps entire quiz UI.", auth: true,
  ...
  // ── Action Tab API: Submit Answer (Step 3 — Per-Question) ──
  { group: "Action Tab API", path: "action-tab-api/submit-answer", method: "POST", desc: "📝 STEP 3 (Optional per question): Record individual answer during an active session. Logs behavioral micro-event for analytics. Call after each question answer for real-time tracking.", auth: true,
    request: { action: "submit-answer", session_id: "uuid-from-step-2", question_id: "q_0_1234567890", selected_option_index: 1, correct_option_index: 1, time_taken_ms: 12000, is_correct: true },
    response: { success: true, is_correct: true, recorded_at: "ISO_DATE" }
  },

  // ── Action Tab API: Complete Focus Session (Step 4 — Results & Impact) ──
  { group: "Action Tab API", path: "action-tab-api/complete-focus-session", method: "POST", desc: "🏆 STEP 4: Complete session — calculates score (4/-1 marking), updates memory_strength, awards XP, generates result card with grade/speed analysis, identifies weak areas, and returns AI-powered recommended_next actions. Single call for entire result screen.", auth: true,
    request: { action: "complete-focus-session", session_id: "uuid-from-step-2", topic_id: "uuid", mode: "focus", duration_minutes: 25, total_questions: 10, answers: [{ question_id: "q_0_1234567890", selected_option_index: 1, correct_option_index: 1, is_correct: true, time_taken_ms: 12000 }, { question_id: "q_1_1234567891", selected_option_index: 0, correct_option_index: 2, is_correct: false, time_taken_ms: 25000 }] },
