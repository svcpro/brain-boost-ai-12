...
const ACRY_API_ROUTES = [
  ...
  // ── Section 1b: Mission Sessions & Streaks (Advanced Wizard APIs) ──
  { group: "Mission Sessions", path: "home-api/mission-session-start", method: "POST", desc: "Create a mission_session row when user enters the 4-step wizard (Read & Recall → Quiz → Apply → Final Review)", auth: true, request: { mission_id: "uuid-or-synthetic", topic_name: "Optics", subject_name: "Physics", estimated_minutes: 10, urgency: "critical" }, response: { success: true, session_id: "uuid", status: "active", steps: [], created_at: "ISO_DATE" } },
  { group: "Mission Sessions", path: "home-api/mission-session-step", method: "POST", desc: "Record a completed step in the wizard. Steps: read_recall (0), quiz (1), apply (2), final_review (3). Includes adaptive difficulty result.", auth: true, request: { session_id: "uuid", step_index: 1, step_type: "quiz", score: 80, accuracy: 0.75, time_seconds: 120, difficulty: "medium", questions_count: 4, correct_count: 3 }, response: { success: true, step_recorded: true, current_step: 2, next_difficulty: "hard", xp_for_step: 15 } },
  { group: "Mission Sessions", path: "home-api/mission-session-complete", method: "POST", desc: "Finalize the mission session after all 4 steps. Calculates brain boost, updates streaks, and returns brain impact report data.", auth: true, request: { session_id: "uuid", total_score: 85, total_accuracy: 0.78, total_time_seconds: 480, xp_earned: 50, difficulty_changes: ["easy", "medium", "hard", "hard"] }, response: { success: true, session: { id: "uuid", status: "completed", xp_earned: 50, brain_boost_pct: 8.5 }, brain_impact: { memory_before: 42, memory_after: 50.5, rank_before: 4500, rank_after: 4380, topic_mastery_before: 35, topic_mastery_after: 48, ai_insight: "Strong performance in quiz phase. Focus on application questions next time." }, streak_updated: true, new_streak: 16 } },
  { group: "Mission Sessions", path: "home-api/mission-session-history", method: "GET", desc: "Get recent mission sessions with scores, XP, and brain boost data for the user", auth: true, request: { limit: 10 }, response: { sessions: [{ id: "uuid", mission_id: "uuid", topic_name: "Optics", score: 85, accuracy: 0.78, xp_earned: 50, brain_boost_pct: 8.5, time_used_seconds: 480, completed_at: "ISO_DATE" }], total_sessions: 42, total_xp: 2150 } },
  { group: "Mission Sessions", path: "home-api/mission-streak", method: "GET", desc: "Get mission-specific streak data — consecutive days with completed missions, longest streak, and freeze status", auth: true, request: {}, response: { current_streak: 16, longest_streak: 42, streak_active_today: true, total_missions_completed: 128, total_xp_earned: 6400, next_milestone: 21, milestone_reward: "🏆 Gold Brain Badge", freezes_available: 2 } },
  { group: "Mission Sessions", path: "home-api/mission-leaderboard", method: "GET", desc: "Get mission completion leaderboard — top users by XP, streak, and missions completed", auth: true, request: { period: "weekly", limit: 20 }, response: { leaderboard: [{ rank: 1, user_id: "uuid", display_name: "Rahul", xp: 350, missions_completed: 7, streak: 21, avatar_url: "https://..." }], user_rank: 5, user_stats: { xp: 200, missions: 5 } } },
  { group: "Mission Sessions", path: "home-api/mission-brain-impact", method: "GET", desc: "Get brain impact report for a specific completed session — memory strength delta, rank change, and AI insights", auth: true, request: { session_id: "uuid" }, response: { memory_strength: { before: 42, after: 50.5, delta: 8.5 }, rank_prediction: { before: 4500, after: 4380, delta: -120 }, topic_mastery: { before: 35, after: 48, delta: 13 }, ai_insight: "Your quiz accuracy improved by 15% compared to last session.", next_mission_preview: { title: "Strengthen: Thermodynamics", type: "strengthen", estimated_minutes: 12 } } },
  { group: "Mission Sessions", path: "home-api/mission-share-data", method: "GET", desc: "Get shareable mission completion card data — formatted for social sharing (text summary + stats)", auth: true, request: { session_id: "uuid" }, response: { share_text: "🧠 Just completed my daily ACRY mission!\n📊 Score: 85% | 🎯 Accuracy: 78%\n🔥 Streak: 16 days | ⚡ +50 XP\n\nStudy smarter with ACRY!", stats: { score: 85, accuracy: 78, streak: 16, xp: 50, brain_boost: 8.5, topic: "Optics" }, share_url: "https://acry.ai/share/uuid" } },
