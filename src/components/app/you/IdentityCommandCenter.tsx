      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <div className={`w-7 h-7 rounded-lg ${consistencyScore >= 65 ? "bg-success/10" : "bg-warning/10"} flex items-center justify-center`}>
              <Activity className={`w-3.5 h-3.5 ${consistencyColor}`} />
            </div>
            <span className="text-[10px] font-medium text-muted-foreground">Consistency</span>
          </div>
          <div className="flex items-baseline gap-1.5 mb-2">
            <span className={`text-xl font-bold ${consistencyColor}`}>{consistencyScore}%</span>
            <span className="text-[10px] text-muted-foreground">{consistencyLabel}</span>
          </div>
          ...
          <div className="flex items-end gap-[3px] h-6">
            {consistencyDays.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                <motion.div
                  className={`w-full rounded-sm ${d.mins > 0 ? "bg-primary/60" : "bg-secondary"}`}
                  initial={{ height: 0 }}
                  animate={{ height: `${Math.max(2, Math.min(24, (d.mins / 60) * 24))}px` }}
          ...
          <div className="flex gap-[3px] mt-0.5">
            {consistencyDays.map((d, i) => (
              <span key={i} className="flex-1 text-center text-[7px] text-muted-foreground">{d.day.charAt(0)}</span>
            ))}
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="text-xl font-bold text-warning">{streakData?.currentStreak ?? 0}</span>
          ...
          <div className="flex items-center gap-1.5 mt-2 pt-2 border-t border-border/20">
            ...
            <span className="text-[10px] text-muted-foreground">
              {freezeCount > 0 ? `${freezeCount} shield${freezeCount > 1 ? "s" : ""} ready` : "No shields"}
            </span>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="glass rounded-xl p-4 neural-border col-span-2"
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-lg ${examReadiness >= 70 ? "bg-success/10" : "bg-accent/10"} flex items-center justify-center`}>
                <Target className={`w-3.5 h-3.5 ${examReadiness >= 70 ? "text-success" : "text-accent"}`} />
              </div>
              <div>
                <span className="text-[10px] font-medium text-muted-foreground block">Exam Readiness</span>
              </div>
            </div>
            <div className="flex items-baseline gap-1.5">
              <span className={`text-xl font-bold ${examReadiness >= 70 ? "text-success" : "text-accent"}`}>{examReadiness}%</span>
              {daysToExam && <span className="text-[10px] text-muted-foreground">{daysToExam}d left</span>}
            </div>
          </div>

          {/* Topic Strength Bars (Radar replacement) */}
          {topicStrengths.length > 0 && (
            <div className="space-y-1.5">
              {topicStrengths.map((t, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="text-[10px] text-muted-foreground w-20 truncate">{t.name}</span>
                  <div className="flex-1 h-1.5 rounded-full bg-secondary overflow-hidden">
                    <motion.div
                      className={`h-full rounded-full ${t.strength >= 70 ? "bg-success" : t.strength >= 40 ? "bg-warning" : "bg-destructive/70"}`}
                      initial={{ width: 0 }}
                      animate={{ width: `${t.strength}%` }}
                      transition={{ duration: 0.6, delay: 0.1 + i * 0.05 }}
                    />
                  </div>
                  <span className="text-[10px] text-muted-foreground w-8 text-right">{t.strength}%</span>
                </div>
              ))}
            </div>
          )}
