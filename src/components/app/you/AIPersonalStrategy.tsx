    ...
    >
        ...
        <div className="glass rounded-2xl p-4 neural-border">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-foreground">Daily Tactical Plan</span>
            </div>
            <motion.div animate={{ rotate: showTactical ? 90 : 0 }} transition={{ duration: 0.2 }}>
            ...
            {showTactical && (
              ...
              >
                <div className="pt-3 space-y-2">
                  {strategy.tactical_plan.map((step, i) => (
                    <motion.div
                      key={i}
                      initial={{ opacity: 0, x: -10 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: i * 0.08 }}
                      className="flex items-start gap-2.5 py-2 px-3 rounded-xl bg-secondary/30 border border-border/20"
                    >
      ...
      <div className="glass rounded-2xl p-4 neural-border">
        ...
        >
          ...
          <motion.div animate={{ rotate: showWhatIf ? 90 : 0 }} transition={{ duration: 0.2 }}>
            <ChevronRight className="w-4 h-4 text-muted-foreground" />
          </motion.div>
        </button>
              ...
              <div className="pt-3 grid grid-cols-2 gap-2">
                {WHAT_IF_SCENARIOS.map((scenario, i) => (
                  <motion.div
                    ...
                    className="rounded-xl bg-secondary/30 border border-border/20 p-3 text-center"
                  >
                    <p className="text-[10px] text-muted-foreground mb-1">{scenario.label}</p>
                    <p className={`text-xs font-bold ${scenario.positive ? "text-success" : "text-destructive"}`}>
                      {scenario.delta}
                    </p>
                  </motion.div>
                ))}
      ...
      {strategy?.adjustments && (
        <div className="glass rounded-2xl p-4 neural-border">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold text-foreground">Auto-Adjustments</span>
          </div>
          ...
          <div className="space-y-1.5 mb-3">
            {strategy.adjustments.map((adj, i) => (
              <div key={i} className="flex items-center gap-2 py-1.5">
                ...
                <span className="text-[11px] text-foreground/80">{adj}</span>
              </div>
            ))}
          </div>
          ...
          <motion.button
            ...
            disabled={autoAdjusting || autoAdjusted}
          ...
          >
            ...
            {autoAdjusting ? "Recalibrating..." : autoAdjusted ? "✓ Strategy Recalibrated" : "Recalibrate Based on Performance"}
          </motion.button>
        </div>
      )}
    </motion.div>
