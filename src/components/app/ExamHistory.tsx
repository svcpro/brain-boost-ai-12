...
import { History, TrendingUp, Zap, Flame, Skull, Trophy, Calendar, Clock } from "lucide-react";
...
const ExamHistory = () => {
  ...
  // Mini chart data (last 10, reversed for chronological)
  ...
  const chartPoints = chartData.map((r, i) => ({
    ...
    y: chartH - ((r.score / r.total_questions) * 100 / chartMax) * (chartH - 6) - 3,
  }));
      ...
      {chartData.length > 1 && (
        <div className="glass rounded-lg p-3 neural-border">
          <svg viewBox={`-5 -3 ${chartW + 10} ${chartH + 6}`} className="w-full h-16">
            ...
            {chartPoints.map((p, i) => {
              const r = chartData[i];
              const diffStyle = DIFF_STYLE[r.difficulty] || DIFF_STYLE.medium;
