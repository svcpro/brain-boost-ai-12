...
const BrainTab = () => {
  const memoryHealth = [
    { subject: "Physics", strength: 78, topics: 24 },
    { subject: "Chemistry", strength: 52, topics: 18 },
    { subject: "Mathematics", strength: 85, topics: 30 },
    { subject: "Biology", strength: 41, topics: 22 },
  ];
    ...
    <div className="px-6 py-6 space-y-6">
        ...
        <div className="relative w-32 h-32 mx-auto mb-4">
          <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
            ...
            <motion.circle
              ...
              animate={{ strokeDashoffset: 264 * (1 - 0.64) }}
              transition={{ duration: 1.5, delay: 0.3 }}
            />
          </svg>
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-3xl font-bold gradient-text">64%</span>
            <span className="text-[10px] text-muted-foreground">Brain Health</span>
          </div>
        </div>
        <p className="text-sm text-muted-foreground">Your memory network is <span className="text-warning font-medium">moderately strong</span></p>
      </motion.div>
      ...
      >
        ...
        <div className="space-y-4">
          {memoryHealth.map((sub, i) => (
            <div key={i}>
              <div className="flex justify-between mb-1.5">
                <span className="text-sm text-foreground">{sub.subject}</span>
              ...
              <p className="text-[10px] text-muted-foreground mt-1">{sub.topics} topics tracked</p>
            </div>
          ))}
        </div>
      </motion.div>
