// Server-side full database backup engine.
...
// - Uploads JSON or NDJSON (one file per table inside a JSON manifest) to Storage.
// - Returns a signed download URL valid for 24h plus run metadata.
...
// Comprehensive table list — anything in `public` schema we want backed up.
const ALL_TABLES = [
...
async function fetchTable(sb: any, table: string) {
  const rows: any[] = [];
  let page = 0;
  while (true) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // List action: return what tables can be backed up
    ...
    const results: Record<string, { count: number; data?: any[]; error?: string }> = {};
    const failed: string[] = [];
    let totalRows = 0;
    let completed = 0;
    ...
    async function worker() {
      while (cursor < requested.length) {
        ...
        try {
          const rows = await fetchTable(sb, table);
          results[table] = { count: rows.length, data: rows };
        ...
        if (completed % 8 === 0 || completed === requested.length) {
          await sb.from("admin_backup_runs")
            .update({ completed_tables: completed, failed_tables: failed, total_rows: totalRows })
            .eq("id", runId);
        }
      }
    }
    ...
    if (format === "ndjson") {
      // NDJSON: one line per (table, row) with table key prefix; great for streaming restore
      ...
      chunks.push(enc.encode(JSON.stringify(meta) + "\n"));
    ...
    const path = `${user.id}/acry-backup-${ts}.${ext}`;
