...
Each topic should be a specific, reviewable concept.`,
    ...
    if (!toolCall?.function?.arguments) {
      return new Response(JSON.stringify({ error: "AI could not extract topics from this image" }), {
    ...
    console.log(`Extracted ${totalTopicsCreated} topics from image`);

    return new Response(JSON.stringify({ success: true, totalTopicsCreated, results }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
    console.error("extract-image-topics error:", e);
