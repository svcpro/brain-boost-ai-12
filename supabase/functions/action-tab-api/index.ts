...
async function handleTodaysGains(userId: string) {
  ...
  yesterdayEndDate.setHours(0, 0, 0, 0);
  yesterdayEndDate.setMilliseconds(-1);

  const [sessionsRes, topicsRes, weekRes, yesterdayRes, todayTopicsRes] = await Promise.all([
    admin.from("study_logs").select("id, duration_minutes, confidence_level, topic_id, created_at, session_type").eq("user_id", userId).gte("created_at", today),
    admin.from("topics").select("id, name, memory_strength, next_review, subject").eq("user_id", userId).is("deleted_at", null),
    admin.from("study_logs").select("duration_minutes, confidence_level, created_at").eq("user_id", userId).gte("created_at", weekAgo),
    admin.from("study_logs").select("duration_minutes, confidence_level").eq("user_id", userId).gte("created_at", yesterdayStartDate.toISOString()).lt("created_at", today),
    admin.from("study_logs").select("topic_id").eq("user_id", userId).gte("created_at", today).not("topic_id", "is", null),
  ]);

  const sessions = (sessionsRes.data || []) as any[];
  const topics = (topicsRes.data || []) as any[];
  const weekSessions = (weekRes.data || []) as any[];
  const yesterdaySessions = (yesterdayRes.data || []) as any[];
  const todayTopicIds = (todayTopicsRes.data || []) as any[];

  const totalMin = sessions.reduce((s: number, r: any) => s + (r.duration_minutes || 0), 0);
  const count = sessions.length;
  const stabilityGain = Math.min(count * 2.5, 15);
  const weakTopicsList = topics.filter((t: any) => (t.memory_strength ?? 0) < 0.4);
  const totalTopics = topics.length || 1;
  const riskReduction = Math.min(count * 3, Math.round((1 - weakTopicsList.length / totalTopics) * 100));
  const rankChange = Math.min(count * 1.5, 10);
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dayEnd = new Date(d); dayEnd.setHours(23, 59, 59, 999);
    const daySessions = weekSessions.filter((s: any) => { const t = new Date(s.created_at).getTime(); return t >= dayStart2.getTime() && t <= dayEnd.getTime(); });
    const mins = daySessions.reduce((sum: number, s: any) => sum + (s.duration_minutes || 0), 0);
    weeklyData.push({ day: dayLabels[d.getDay()], value: mins, sessions: daySessions.length });
  }
  let focusStreak = 0;
  ...
  const topicsStudied = uniqueTopicIds.map((id) => {
  ...
  const bestSessionMinutes = sessions.length > 0 ? Math.max(...sessions.map((s: any) => s.duration_minutes || 0)) : 0;
  ...
  sessions.forEach((s: any) => { sessionTypes[s.session_type || "study"] = (sessionTypes[s.session_type || "study"] || 0) + 1; });
  ...
  const todayVsYesterday = {
    ...
    sessions_diff: count - yesterdaySessions.length,
  ...
  const weakTopicsDetail = weakTopicsList
    .sort((a: any, b: any) => (a.memory_strength ?? 0) - (b.memory_strength ?? 0))
    .slice(0, 5)
    .map((t: any) => ({ id: t.id, name: t.name, subject: t.subject, memory_strength: t.memory_strength ?? 0, overdue: t.next_review ? new Date(t.next_review) < now : false }));

  const sessionTimeline = sessions.map((s: any) => ({
    id: s.id, duration_minutes: s.duration_minutes || 0, confidence_level: s.confidence_level || "unrated",
    session_type: s.session_type || "study", topic_id: s.topic_id, created_at: s.created_at,
  }));
  ...
  return {
    ...
    weak_topics_count: weakTopicsList.length,
    total_topics_count: topics.length,
    weak_topics: weakTopicsDetail,
    confidence_breakdown: confidenceBreakdown,
    session_types: sessionTypes,
    today_vs_yesterday: todayVsYesterday,
    session_timeline: sessionTimeline,
    weekly_summary: { total_minutes: weekTotalMin, total_sessions: weekSessions.length, avg_daily_minutes: Math.round(weekTotalMin / 7) },
